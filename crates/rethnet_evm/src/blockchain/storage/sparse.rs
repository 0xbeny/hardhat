...
pub struct SparseBlockchainStorage {
    ...
    number_to_block: HashMap<U256, Arc<Block>>,
    transaction_hash_to_block: HashMap<B256, Arc<Block>>,
}

impl SparseBlockchainStorage {
    /// Constructs a new instance with the provided block.
    pub fn with_block(block: Block, total_difficulty: U256) -> Self {
    ...
    pub fn block_by_number(&self, number: &U256) -> Option<&Arc<Block>> {
        self.number_to_block.get(number)
    }

    /// Retrieves the block by hash, if it exists.
    pub fn block_by_transaction_hash(&self, transaction_hash: &B256) -> Option<&Arc<Block>> {
